...
def edit_profile():
    ...
    output = {'email':current_user, 'phone':_phone, 'fullname':_fullname, 'message':'ok'}
...
def add_beneficiary():
    ...
    bindData = (_account_name, _account_number, _currency, _beneficiary_name,_destination_branch_code,current_user)
    cursor.execute(query,bindData)  
    mysql.connection.commit()
    cursor.close()
    output = {'message':'ok'}
...
def create_local_transfer():
    ...
    data = {'account_bank':_account_bank,'account_number':_account_number,'amount':_amount,'narration':_narration,'currency':_currency,'reference':_reference,'debit currency':_debit_currency}
    res = requests.post(url, data=json.dumps(data),headers={'Content-Type':'application/json','Authorization':'Bearer FLWSECK_TEST-72fe360edef17334f4817a17407011bb-X'})
...
def create_intl_transfer():
    ...
    url = 'https://api.flutterwave.com/v3/transfers'
    data = {'account_bank':_account_bank,'account_number':_account_number,'destination_branch_code':_destination_branch_code,'amount':_amount,'narration':_narration,'currency':_currency,'reference':_reference,'beneficiary_name':_beneficiary_name,'debit currency':_debit_currency}
    res = requests.post(url, data=json.dumps(data),headers={'Content-Type':'application/json','Authorization':'Bearer FLWSECK_TEST-72fe360edef17334f4817a17407011bb-X'})
